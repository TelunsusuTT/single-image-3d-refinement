"""Protocol-corrected data and trainable-scope support."""
...
from .dataset import ProtocolCorrectedTextureDataset
...
ALLOWED_TRAINABLE_SCOPES = frozenset({"broad_scope", "multi_view_attention"})
_MVA_PROJECTION_SUFFIXES = {
    ("to_q",),
    ("to_k",),
    ("to_v",),
    ("to_out", "0"),
...
class ProtocolCorrectedDatasetFromJson:
    """Config-friendly JSON wrapper around the protocol-corrected dataset reader."""
...
def protocol_collate_fn(batch: Sequence[Mapping[str, object]]) -> dict[str, object]:
    ...
    for index, sample in enumerate(batch):
        ...
        if missing:
            raise KeyError(f"Batch item {index} is missing model input keys: {missing}")
        ...
        model_samples.append({key: sample[key] for key in MODEL_INPUT_KEYS})
...
def _is_mva_projection_module(module_name: str) -> bool:
    segments = tuple(module_name.split("."))
    for index, segment in enumerate(segments):
        if segment == "attn_multiview" and segments[index + 1 :] in _MVA_PROJECTION_SUFFIXES:
            return True
    return False


def _forbidden_mva_segment(parameter_name: str) -> str | None:
...
def _select_mva_parameters(model: Any) -> list[tuple[str, Any]]:
    ...
    for module_name, module in _named_modules_with_duplicates(model):
        if not _is_mva_projection_module(module_name):
            continue
        if not isinstance(module, nn.Linear):
            raise TypeError(f"Multi-View-Attention projection owner must be nn.Linear: {module_name} -> {type(module).__name__}")
        for leaf_name in ("weight", "bias"):
            parameter = getattr(module, leaf_name, None)
            if parameter is None:
                continue
            if not isinstance(parameter, nn.Parameter):
                raise TypeError(f"Multi-View-Attention {module_name}.{leaf_name} is not an nn.Parameter")
            full_name = f"{module_name}.{leaf_name}"
            if parameter_by_name.get(full_name) is not parameter:
                raise RuntimeError(f"Multi-View-Attention selected parameter is not registered at its exact path: {full_name}")
            forbidden = _forbidden_mva_segment(full_name)
            if forbidden is not None:
                raise ValueError(f"Multi-View-Attention selected forbidden path segment {forbidden!r}: {full_name}")
            aliases = names_by_identity[id(parameter)]
            if len(aliases) != 1:
                raise ValueError(f"Duplicate Multi-View-Attention parameter identity for {full_name}: aliases={aliases}")
            selected.append((full_name, parameter))

    if not selected:
        raise ValueError("Multi-View-Attention matched zero attn_multiview q/k/v/output projection parameters")
    selected_ids = [id(parameter) for _, parameter in selected]
    if len(selected_ids) != len(set(selected_ids)):
        raise ValueError("Multi-View-Attention selected duplicate parameter identities")
    return selected


def apply_trainable_scope(model: Any, scope: str) -> TrainableScopeReport:
    """Apply Broad-Scope preservation or fail-closed Multi-View-Attention projection selection."""
    ...
    if scope == "broad_scope":
        report = _scope_report(model, scope)
        if report.trainable_parameter_tensor_count == 0:
            raise ValueError("Broad-Scope found zero parameters already marked trainable")
        return report

    selected = _select_mva_parameters(model)
    ...
    if set(report.trainable_parameter_names) != set(selected_names):
        raise RuntimeError("Multi-View-Attention report does not exactly match selected parameter names")
    return report
