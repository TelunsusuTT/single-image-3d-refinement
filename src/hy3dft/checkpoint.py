"""Strict loading for protocol-corrected trainable-scope checkpoints.

This scope-only artifact contract is distinct from the complete UNet state used
by the early Broad-Scope inference backend. Static manifest validation uses only
the Python standard library; Torch is imported lazily by the runtime loader.
"""
...
CHECKPOINT_FORMAT = "trainable_scope_checkpoint_v1"
ALLOWED_SCOPES = frozenset({"multi_view_attention", "broad_scope"})
...
def validate_checkpoint_artifact(
    ...
    *,
    ...
    if not isinstance(recorded_path, str) or not recorded_path.strip():
        raise ValueError("Checkpoint manifest checkpoint_path must record non-empty provenance")
    ...
    return {
        ...
        "manifest_recorded_checkpoint_path": recorded_path,
...
def derive_expected_scope(
    ...
    *,
    ...
    Broad-Scope preserves the official training initializer's trainable subset. An
    ...
    try:
        if scope == "broad_scope" and audited is not None:
            report = dict(audited)
            resolution_basis = "audited_official_training_scope_report"
        else:
            if scope_resolver is None:
                from hy3dft.adaptation import apply_trainable_scope
...
def load_scope_checkpoint_into_model(
    ...
    *,
    ...
    validated_items: list[tuple[str, Any, Any]] = []
    for name, source in items:
        ...
        validated_items.append((name, source, target))

    copied_names: list[str] = []
    with torch_module.no_grad():
        for name, source, target in validated_items:
    ...
    return {
        ...
        "manifest_recorded_checkpoint_path": static[
            "manifest_recorded_checkpoint_path"
        ],
