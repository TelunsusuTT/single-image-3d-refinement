#!/usr/bin/env python3
"""Static readiness checks for Data v2 mini40 true-PBR training."""
...
def check_examples_json(path: Path, expected_count: int, label: str, errors: list[str]) -> dict[str, Any]:
    ...
    for value in data:
        ...
        sample_errors.extend(f"missing file: {path}" for path in missing_files)
...
def check_checkpoint_dir(config: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    ...
    old_tokens = ["pilot_v1_overfit_500", "pilot_v1_conservative_50_lr1e6", "pilot_v1_truepbr"]
    if any(token in str(resolved) for token in old_tokens):
        errors.append(f"checkpoint dir appears to be an old pilot_v1 dir: {resolved}")
    return {"path": str(resolved), "exists_or_created": resolved.is_dir(), "existing_ckpts": existing_ckpts}


def check_env_and_hypaint(config: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    env_sh = PROJECT_ROOT / "env" / "env.sh"
    hypaint = Path(config.get("hypaint_dir", "")).expanduser()
    if not hypaint.is_absolute():
        hypaint = PROJECT_ROOT / hypaint
    return {
        "env_sh": check_file(env_sh, "env/env.sh", errors),
        "hypaint_dir": check_dir(hypaint, "HYPAINT dir", errors),
        "hypaint_train_py": check_file(hypaint / "train.py", "HYPAINT train.py", errors),
    }


def check_split_membership(config: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    path = resolve_project_path(config.get("split_membership_csv", ""))
    rows = read_csv(path)
    mini_rows = [row for row in rows if row.get("split_config") == "mini40"]
    seen: dict[str, str] = {}
    duplicates: list[str] = []
    for row in mini_rows:
        ...
        if item_id in seen:
            ...
            errors.append(f"mini40 split membership duplicates item_id: {item_id}")
        seen[item_id] = split
    return {"path": str(path), "exists": path.is_file(), "mini40_rows": len(mini_rows), "duplicates": duplicates}
...
def readiness_report(config_path: Path) -> dict[str, Any]:
    ...
    report["env"] = check_env_and_hypaint(config, errors)
    report["split_membership"] = check_split_membership(config, errors)
...
def write_reports(report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Phase 2L.4A Mini40 Training Readiness",
        "",
...
def print_summary(report: dict[str, Any]) -> None:
    config = report.get("config", {})
    print("Phase 2L.4A mini40 training readiness")
    ...
    if report["ok"]:
        print("PHASE2L4A_MINI40_TRAINING_READINESS_OK")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check Data v2 frame mini40 training readiness.")
