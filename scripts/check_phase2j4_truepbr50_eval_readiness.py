#!/usr/bin/env python3
"""Check Phase 2J.4 true-PBR 50-step evaluation readiness."""
...
TRUEPBR50_TOKEN = "pilot_v1_truepbr_50_lr1e6"
...
def check_readiness(
    ...
    checkpoint_has_truepbr50_token = TRUEPBR50_TOKEN in checkpoint_text
    forbidden_checkpoint_hits = [
        token for token in FORBIDDEN_CHECKPOINT_TOKENS if token in checkpoint_text
    ]
    if not checkpoint_has_truepbr50_token:
        errors.append(f"checkpoint path does not contain {TRUEPBR50_TOKEN}")
    ...
    outputs = {
        ...
        "delta_output_dir": check_parent(delta_output_dir, "delta-output-dir", errors),
    ...
    return {
        ...
        "checkpoint_has_truepbr50_token": checkpoint_has_truepbr50_token,
...
def print_summary(report: dict[str, Any]) -> None:
    print("Phase 2J.4 true-PBR 50-step evaluation readiness")
    print(f"  case_dir: {report['case_dir']}")
    print(f"  base_dir: {report['base_dir']}")
    print(f"  checkpoint: {report['checkpoint']}")
    print(f"  hypaint: {report['hypaint']}")
    print(f"  checkpoint_has_truepbr50_token: {report['checkpoint_has_truepbr50_token']}")
    ...
    if report["ok"]:
        print("PHASE2J4_TRUEPBR50_EVAL_PREFLIGHT_OK")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check Phase 2J.4 true-PBR 50-step evaluation readiness."
    )
    ...
    parser.add_argument("--delta-output-dir", required=True, type=Path)
