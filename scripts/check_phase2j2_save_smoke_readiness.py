#!/usr/bin/env python3
"""Check Phase 2J.2 true-PBR checkpoint save smoke readiness."""
...
def check_examples(examples_json: Path, errors: list[str]) -> dict[str, Any]:
    ...
    for index, value in enumerate(data, start=1):
        ...
        if not sample_dir.is_dir():
            sample_error.append("sample directory missing")
        if not render_tex.is_dir():
            sample_error.append("render_tex missing")
        if not render_cond.is_dir():
            sample_error.append("render_cond missing")
        errors.extend(f"{sample_dir}: {error}" for error in sample_error)
    ...
    if len(sample_dirs) != EXPECTED_SAMPLE_COUNT:
        errors.append(
            f"sample count {len(sample_dirs)} != expected {EXPECTED_SAMPLE_COUNT}"
        )
    return {
        **result,
...
def check_config(config: Path, errors: list[str]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "has_base_learning_rate_0": False,
        "has_max_steps_1": False,
        "has_every_n_train_steps_1": False,
    ...
    if result["exists"]:
        ...
        checks["has_base_learning_rate_0"] = contains_scalar(text, "base_learning_rate", "0.0")
        checks["has_max_steps_1"] = contains_scalar(text, "max_steps", "1")
        checks["has_every_n_train_steps_1"] = contains_scalar(text, "every_n_train_steps", "1")
        checks["has_save_weights_only_true"] = contains_scalar(text, "save_weights_only", "true")
        ...
        if not checks["has_base_learning_rate_0"]:
            errors.append("config does not contain base_learning_rate: 0.0")
        if not checks["has_max_steps_1"]:
            errors.append("config does not contain max_steps: 1")
        if not checks["has_every_n_train_steps_1"]:
            errors.append("config does not contain every_n_train_steps: 1")
...
def check_readiness(
    ...
    report = {
        ...
        "output_dir": check_output_dir(output_dir, errors),
    }
...
def print_summary(report: dict[str, Any]) -> None:
    print("Phase 2J.2 save smoke readiness")
    ...
    print(f"  config_has_base_learning_rate_0: {config['has_base_learning_rate_0']}")
    print(f"  config_has_max_steps_1: {config['has_max_steps_1']}")
    print(f"  config_has_every_n_train_steps_1: {config['has_every_n_train_steps_1']}")
    ...
    if report["ok"]:
        print("PHASE2J2_SAVE_SMOKE_PREFLIGHT_OK")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check Phase 2J.2 save smoke readiness.")
