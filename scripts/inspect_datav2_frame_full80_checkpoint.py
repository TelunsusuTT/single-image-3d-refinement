#!/usr/bin/env python3
"""Stat-only inspection for Data v2 full80 true-PBR checkpoints."""
...
def inspect_checkpoint_dir(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "checkpoints": files,
        "ok": len(files) == 1 and len(expected) == 1,
    }
...
def write_reports(config: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Phase 2L.6B Full80 Checkpoint Inspection",
        "",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inspect full80 true-PBR checkpoint files without loading them.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Phase 2L.6B full80 checkpoint inspection")
    ...
    if report["ok"]:
        print("PHASE2L6B_FULL80_CHECKPOINT_INSPECTION_OK")
        return 0
    return 1
