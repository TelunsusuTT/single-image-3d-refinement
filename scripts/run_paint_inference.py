#!/usr/bin/env python3
"""Fixed-mesh Hunyuan3D-Paint inference backend."""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path
from typing import Any


PROJECT_ROOT = Path(__file__).resolve().parents[1]
SRC_ROOT = PROJECT_ROOT / "src"
if str(SRC_ROOT) not in sys.path:
    sys.path.insert(0, str(SRC_ROOT))

from hy3dft.inference import (  # noqa: E402
    initialize_base_paint_pipeline,
    resolve_official_paths,
    run_paint_inference,
)


FULL_UNET_CHECKPOINT_PREFIX = "unet."
FULL_UNET_CHECKPOINT_TARGET_PATH = "paint_pipeline.models['multiview_model'].pipeline.unet"


def nonzero_file(path: Path, label: str, errors: list[str]) -> Path:
    resolved = path.expanduser().resolve()
    if not resolved.is_file():
        errors.append(f"{label} missing: {resolved}")
    elif resolved.stat().st_size <= 0:
        errors.append(f"{label} is zero-size: {resolved}")
    return resolved


def validate_inputs(args: argparse.Namespace) -> tuple[dict[str, Any], list[str]]:
    case_dir = args.case_dir.expanduser().resolve()
    output_dir = args.output_dir.expanduser().resolve()
    errors: list[str] = []
    if args.seed < 0 or args.seed > 2**32 - 1:
        errors.append("--seed must be between 0 and 4294967295")
    if output_dir.exists():
        errors.append(f"output-dir already exists; choose a new directory: {output_dir}")
    if not case_dir.is_dir():
        errors.append(f"case-dir missing: {case_dir}")
    mesh = nonzero_file(case_dir / "input" / "mesh.glb", "input mesh", errors)
    image = nonzero_file(case_dir / "input" / "image.png", "input image", errors)

    checkpoint = None
    if args.mode == "checkpoint":
        if args.checkpoint is None:
            errors.append("--checkpoint is required for --mode checkpoint")
        else:
            checkpoint = nonzero_file(args.checkpoint, "checkpoint", errors)
    elif args.checkpoint is not None:
        checkpoint = args.checkpoint.expanduser().resolve()

    plan = {
        "case_dir": str(case_dir),
        "input_mesh": str(mesh),
        "input_image": str(image),
        "output_dir": str(output_dir),
        "planned_output_mesh": str(output_dir / "textured_mesh.obj"),
        "planned_output_glb": str(output_dir / "textured_mesh.glb"),
        "mode": args.mode,
        "checkpoint": str(checkpoint) if checkpoint is not None else "",
        "conditioning_view": args.conditioning_view,
        "seed": args.seed,
        "max_num_view": args.max_num_view,
        "resolution": args.resolution,
        "device": args.device,
        "use_remesh": not args.no_remesh,
        "dry_run": args.dry_run,
    }
    return plan, errors


def write_plan(plan: dict[str, Any]) -> None:
    output_dir = Path(plan["output_dir"])
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "run_plan.json").write_text(
        json.dumps(plan, indent=2),
        encoding="utf-8",
    )


def run_dry_run(plan: dict[str, Any]) -> int:
    Path(plan["output_dir"]).mkdir(parents=True, exist_ok=False)
    write_plan(plan)
    print("Paint-stage inference dry run")
    for key in (
        "mode",
        "input_mesh",
        "input_image",
        "output_dir",
        "planned_output_mesh",
        "planned_output_glb",
        "checkpoint",
        "conditioning_view",
        "seed",
        "max_num_view",
        "resolution",
        "device",
        "use_remesh",
    ):
        print(f"  {key}: {plan[key]}")
    print("PAINT_INFERENCE_DRY_RUN_OK")
    return 0


def torch_load_cpu(torch_module: Any, checkpoint: Path) -> Any:
    try:
        return torch_module.load(checkpoint, map_location="cpu", weights_only=True)
    except TypeError as exc:
        raise RuntimeError("Torch with weights_only checkpoint loading is required") from exc


def extract_state_dict(obj: Any) -> dict[str, Any]:
    if isinstance(obj, dict) and isinstance(obj.get("state_dict"), dict):
        return obj["state_dict"]
    if isinstance(obj, dict):
        return obj
    return {}


def transformed_state_dict(state: dict[str, Any], prefix: str) -> dict[str, Any]:
    transformed: dict[str, Any] = {}
    for key, value in state.items():
        text_key = str(key)
        if text_key.startswith(prefix):
            transformed[text_key[len(prefix):]] = value
    return transformed


def shape_of(value: Any) -> list[int] | None:
    shape = getattr(value, "shape", None)
    return list(shape) if shape is not None else None


def verify_state_compatibility(transformed: dict[str, Any], target_state: dict[str, Any]) -> None:
    transformed_keys = set(transformed.keys())
    target_keys = set(target_state.keys())
    if transformed_keys != target_keys:
        missing = sorted(target_keys - transformed_keys)[:20]
        extra = sorted(transformed_keys - target_keys)[:20]
        raise RuntimeError(
            "Checkpoint key set does not match inference target. "
            f"missing_sample={missing} extra_sample={extra}"
        )
    mismatches = []
    for key in sorted(target_keys):
        checkpoint_shape = shape_of(transformed[key])
        target_shape = shape_of(target_state[key])
        if checkpoint_shape != target_shape:
            mismatches.append((key, checkpoint_shape, target_shape))
            if len(mismatches) >= 20:
                break
    if mismatches:
        raise RuntimeError(f"Checkpoint tensor shape mismatches: {mismatches}")


def load_full_unet_checkpoint_into_pipeline(checkpoint: Path, paint_pipeline: Any) -> dict[str, Any]:
    import torch  # type: ignore

    target = paint_pipeline.models["multiview_model"].pipeline.unet
    target_state = target.state_dict()
    checkpoint_obj = torch_load_cpu(torch, checkpoint)
    state = extract_state_dict(checkpoint_obj)
    transformed = transformed_state_dict(state, FULL_UNET_CHECKPOINT_PREFIX)
    verify_state_compatibility(transformed, target_state)
    load_result = target.load_state_dict(transformed, strict=True)
    return {
        "checkpoint": str(checkpoint),
        "target_path": FULL_UNET_CHECKPOINT_TARGET_PATH,
        "checkpoint_prefix": FULL_UNET_CHECKPOINT_PREFIX,
        "raw_state_dict_key_count": len(state),
        "transformed_key_count": len(transformed),
        "target_key_count": len(target_state),
        "missing_keys": list(getattr(load_result, "missing_keys", [])),
        "unexpected_keys": list(getattr(load_result, "unexpected_keys", [])),
    }


def seed_runtime(seed: int) -> dict[str, Any]:
    """Reset Python, NumPy, and Torch immediately before inference."""

    import random

    import numpy as np
    import torch  # type: ignore

    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    cuda_available = bool(torch.cuda.is_available())
    if cuda_available:
        torch.cuda.manual_seed_all(seed)
    return {
        "seed": seed,
        "python_random_seeded": True,
        "numpy_random_seeded": True,
        "torch_cpu_seeded": True,
        "torch_cuda_seeded": cuda_available,
    }


def run_real_inference(args: argparse.Namespace, plan: dict[str, Any]) -> int:
    hunyuan_root, paint_root = resolve_official_paths()

    output_dir = Path(plan["output_dir"])
    output_dir.mkdir(parents=True, exist_ok=False)
    plan["resolved_hunyuan3d_root"] = str(hunyuan_root)
    plan["resolved_hunyuan3d_paint_root"] = str(paint_root)
    write_plan(plan)

    # The shared helper imports official modules only inside this real runtime
    # branch. Dry-run and static tests remain Hunyuan/Torch free.
    paint_pipeline, runtime_metadata = initialize_base_paint_pipeline(
        max_num_view=args.max_num_view,
        resolution=args.resolution,
        device=args.device,
    )
    plan.update(runtime_metadata)
    write_plan(plan)

    print("Paint-stage inference")
    print(f"  mode: {args.mode}")
    print(f"  conditioning_view: {args.conditioning_view}")
    print(f"  seed: {args.seed}")
    print(f"  Hunyuan3D root: {hunyuan_root}")
    print(f"  Hunyuan3D-Paint root: {paint_root}")
    print(f"  multiview_cfg_path: {plan['multiview_cfg_path']}")
    print(f"  realesrgan_ckpt_path: {plan['realesrgan_ckpt_path']}")
    print(f"  mesh: {plan['input_mesh']}")
    print(f"  image: {plan['input_image']}")
    print(f"  checkpoint: {plan['checkpoint']}")
    print(f"  output_mesh: {plan['planned_output_mesh']}")
    print(f"  planned_output_glb: {plan['planned_output_glb']}")
    print(f"  use_remesh: {plan['use_remesh']}")

    if args.mode == "checkpoint":
        if args.checkpoint is None:
            raise RuntimeError("--checkpoint is required for --mode checkpoint")
        load_summary = load_full_unet_checkpoint_into_pipeline(args.checkpoint.expanduser().resolve(), paint_pipeline)
        plan["checkpoint_load"] = load_summary
        write_plan(plan)
        print(f"  checkpoint_target_path: {load_summary['target_path']}")
        print(f"  transformed_key_count: {load_summary['transformed_key_count']}")
        print(f"  target_key_count: {load_summary['target_key_count']}")
        print("CHECKPOINT_LOADED_OK")

    plan["runtime_seeding"] = seed_runtime(args.seed)
    write_plan(plan)
    print("INFERENCE_SEED_RESET_OK")

    result = run_paint_inference(
        paint_pipeline,
        mesh_path=plan["input_mesh"],
        image_path=plan["input_image"],
        output_mesh_path=plan["planned_output_mesh"],
        use_remesh=plan["use_remesh"],
        save_glb=True,
    )
    output_glb = Path(plan["planned_output_glb"])
    if not output_glb.is_file() or output_glb.stat().st_size <= 0:
        raise RuntimeError(f"Paint-stage inference did not create its GLB: {output_glb}")
    plan["actual_output_mesh"] = str(result)
    plan["actual_output_glb"] = str(output_glb)
    write_plan(plan)
    print(f"Output mesh path: {result}")
    print(f"Output GLB path: {output_glb}")
    return 0


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fixed-mesh Hunyuan3D-Paint inference backend.")
    parser.add_argument("--case-dir", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--mode", required=True, choices=("baseline", "checkpoint"))
    parser.add_argument("--checkpoint", type=Path)
    parser.add_argument(
        "--conditioning-view",
        choices=("000", "001", "002", "003", "004", "005"),
        default="005",
        help="View identity already selected as input/image.png.",
    )
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--max-num-view", type=int, default=6)
    parser.add_argument("--resolution", type=int, default=512)
    parser.add_argument("--device", default="cuda")
    parser.add_argument(
        "--no-remesh",
        action="store_true",
        help="Disable official remeshing and keep input geometry fixed.",
    )
    parser.add_argument("--dry-run", action="store_true")
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    plan, errors = validate_inputs(args)
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    if args.dry_run:
        return run_dry_run(plan)
    return run_real_inference(args, plan)


if __name__ == "__main__":
    raise SystemExit(main())
