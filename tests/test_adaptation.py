...
from hy3dft.adaptation import (  # noqa: E402
    MODEL_INPUT_KEYS,
...
def test_json_wrapper_loads_absolute_list_and_delegates_to_protocol_dataset(tmp_path: Path) -> None:
...
def test_wrapper_returns_model_keys_shapes_and_protocol_metadata(tmp_path: Path) -> None:
    ...
    assert set(sample) == set(MODEL_INPUT_KEYS) | {"protocol_metadata"}
...
def expected_mva_names(model: nn.Module) -> set[str]:
    return {
        name
...
def test_broad_scope_preserves_existing_requires_grad_state_exactly() -> None:
    ...
    report = apply_trainable_scope(model, "broad_scope")
    ...
    assert json.loads(json.dumps(report.to_dict()))["scope"] == "broad_scope"
    with pytest.raises(FrozenInstanceError):
        report.scope = "multi_view_attention"  # type: ignore[misc]


def test_broad_scope_rejects_zero_trainable_state_without_unfreezing() -> None:
    model = FakeHunyuanModel()
    for parameter in model.parameters():
        parameter.requires_grad = False

    with pytest.raises(ValueError, match="zero"):
        apply_trainable_scope(model, "broad_scope")
    assert not any(parameter.requires_grad for parameter in model.parameters())


def test_multi_view_attention_trains_only_exact_multiview_linear_projections() -> None:
    model = FakeHunyuanModel()
    expected = expected_mva_names(model)

    report = apply_trainable_scope(model, "multi_view_attention")
...
def test_multi_view_attention_rejects_empty_match_and_non_linear_owner() -> None:
    with pytest.raises(ValueError, match="zero"):
        apply_trainable_scope(nn.Sequential(nn.Linear(4, 4)), "multi_view_attention")

    model = FakeHunyuanModel()
    model.unet.block.attn_multiview.to_q = nn.Conv2d(4, 4, 1)
    with pytest.raises(TypeError, match="nn.Linear"):
        apply_trainable_scope(model, "multi_view_attention")


def test_multi_view_attention_rejects_duplicate_parameter_identity() -> None:
    model = FakeHunyuanModel()
    model.unet.block.attn_multiview.to_k.weight = model.unet.block.attn_multiview.to_q.weight

    with pytest.raises(ValueError, match="Duplicate Multi-View-Attention parameter identity"):
        apply_trainable_scope(model, "multi_view_attention")


def test_multi_view_attention_rejects_forbidden_selected_prefix_and_unimplemented_scopes() -> None:
    model = nn.Module()
    ...
    with pytest.raises(ValueError, match="Unsupported"):
        apply_trainable_scope(FakeHunyuanModel(), "unsupported_scope")
    with pytest.raises(ValueError, match="Unsupported"):
        apply_trainable_scope(FakeHunyuanModel(), "another_unsupported_scope")
