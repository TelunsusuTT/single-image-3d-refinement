...
from scripts import make_datav2_frame_full80_input_view_review as review
...
def write_split_csv(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    rows = [
        {"split_config": "full101", "split": "val", "item_id": "VAL1", "local_glb_path": "/tmp/VAL1.glb", "selected_input_view": "005"},
        {"split_config": "full101", "split": "test", "item_id": "TEST1", "local_glb_path": "/tmp/TEST1.glb", "selected_input_view": "005"},
        {"split_config": "full101", "split": "train", "item_id": "TRAIN1", "local_glb_path": "/tmp/TRAIN1.glb", "selected_input_view": "005"},
        {"split_config": "mini40", "split": "val", "item_id": "MINI_VAL", "local_glb_path": "/tmp/MINI_VAL.glb", "selected_input_view": "005"},
    ]
...
def test_input_view_review_creates_board_and_default_override_csv(tmp_path):
    ...
    assert data["primary_eval_asset_count"] == 2
    rows = list(csv.DictReader(override_csv.open("r", encoding="utf-8", newline="")))
    assert {row["item_id"] for row in rows} == {"VAL1", "TEST1", "TRAIN1"}
