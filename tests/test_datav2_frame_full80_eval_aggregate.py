...
from scripts import aggregate_datav2_frame_full80_eval as aggregate
...
def make_setup(tmp_path: Path) -> Path:
    ...
    config = {"experiment_name": "full80_eval", "output_root": str(output_root), "primary_front_views": ["004", "005"]}
...
def test_aggregate_rendered_metrics_reports_split_and_view_groups(tmp_path):
    ...
    report = json.loads((tmp_path / "outputs" / "summary" / "full80_eval_summary.json").read_text(encoding="utf-8"))
    ...
    assert "mini40_comparison_note" in report
