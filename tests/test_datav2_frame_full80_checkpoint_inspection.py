...
from scripts import inspect_datav2_frame_full80_checkpoint as inspect_checkpoint
...
def test_checkpoint_inspection_reports_exactly_one_expected_ckpt(tmp_path):
    checkpoint_dir = tmp_path / "checkpoints"
    checkpoint_dir.mkdir()
    ckpt = checkpoint_dir / "datav2_frame_full80_truepbr_500_lr1e6-step500.ckpt"
...
def test_checkpoint_inspection_fails_without_expected_step_marker(tmp_path):
    checkpoint_dir = tmp_path / "checkpoints"
    checkpoint_dir.mkdir()
    (checkpoint_dir / "datav2_frame_full80_truepbr_500_lr1e6-step499.ckpt").write_bytes(b"x")
    config_json = write_config(tmp_path, checkpoint_dir)
...
def test_checkpoint_inspection_fails_with_two_expected_ckpts(tmp_path):
    checkpoint_dir = tmp_path / "checkpoints"
    checkpoint_dir.mkdir()
    (checkpoint_dir / "datav2_frame_full80_truepbr_500_lr1e6-step500.ckpt").write_bytes(b"x")
    (checkpoint_dir / "datav2_frame_full80_truepbr_500_lr1e6-stepstep=500.ckpt").write_bytes(b"x")
    config_json = write_config(tmp_path, checkpoint_dir)

    assert inspect_checkpoint.main(["--config", str(config_json)]) == 1
    report = json.loads((tmp_path / "reports" / "checkpoint_inspection.json").read_text(encoding="utf-8"))
    assert report["checkpoint_count"] == 2
    assert report["expected_checkpoint_count"] == 2
