...
from check_phase2j3_truepbr50_readiness import main as readiness_main  # noqa: E402
    ...
    return write_text(
        ...
        "\n".join(
            [
                ...
                "  modelcheckpoint:",
                "    params:",
                "      dirpath: /tmp/checkpoints/pilot_v1_truepbr_50_lr1e6",
                "      filename: pilot_v1_truepbr_50_lr1e6-step{step}",
...
def make_valid_setup(root: Path) -> tuple[Path, Path, Path, Path]:
    ...
    checkpoint_root = root / "checkpoints" / "pilot_v1_truepbr_50_lr1e6"
    return examples_json, config, hypaint, checkpoint_root
...
def test_readiness_fails_if_config_contains_sd2_community() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        examples_json, config, hypaint, checkpoint_root = make_valid_setup(root)
        write_config(config, extra_text="old: sd2-community/stable-diffusion-2-1")
        assert run_readiness(examples_json, config, hypaint, checkpoint_root) == 1


def test_readiness_fails_if_learning_rate_is_not_1e6() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        examples_json, config, hypaint, checkpoint_root = make_valid_setup(root)
        write_config(config, learning_rate="0.0")
        assert run_readiness(examples_json, config, hypaint, checkpoint_root) == 1


def test_readiness_fails_if_max_steps_is_not_50() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        examples_json, config, hypaint, checkpoint_root = make_valid_setup(root)
        write_config(config, max_steps=1)
        assert run_readiness(examples_json, config, hypaint, checkpoint_root) == 1
