...
from scripts import check_datav2_frame_full80_eval_readiness as readiness


VIEW_IDS = ["000", "001", "002", "003", "004", "005"]


def make_readiness_setup(tmp_path: Path, include_override: bool = True, expected_total: int = 1) -> Path:
    output_root = tmp_path / "outputs"
    sample_root = tmp_path / "samples"
    case_dir = output_root / "cases" / "val" / "VAL1"
    checkpoint = tmp_path / "datav2_frame_full80_truepbr_500_lr1e6-stepstep=500.ckpt"
    ...
    config = {
        ...
        "expected_primary_eval_case_count": 1,
...
def test_eval_readiness_passes_for_valid_fake_case(tmp_path):
    ...
    assert report["primary_eval_case_count"] == 1
...
def test_eval_readiness_fails_on_expected_case_count_mismatch(tmp_path):
    config_path = make_readiness_setup(tmp_path, expected_total=2)

    assert readiness.main(["--config", str(config_path)]) == 1
    report = json.loads((tmp_path / "outputs" / "readiness_summary.json").read_text(encoding="utf-8"))
    assert any("total eval case count 1 != expected 2" in error for error in report["errors"])
